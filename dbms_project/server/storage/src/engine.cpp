...
void Engine::execute(const QueryPlan& plan) {
    ...
    else throw std::runtime_error("Операция еще не реализована");
}
...
void Engine::create_table(const QueryPlan& plan) {
    ...
    TableHeader header; std::memset(&header, 0, sizeof(header));
    ...
    char buffer[PAGE_SIZE] = {0}; std::memcpy(buffer, &header, sizeof(header));
    pager.write_page(0, buffer); pager.allocate_page();
}
...
void Engine::insert_record(const QueryPlan& plan) {
    ...
    Pager pager(table_path.string()); Pager idx_pager(idx_path.string());
    char header_buffer[PAGE_SIZE]; pager.read_page(0, header_buffer);
    ...
    for (size_t i = 0; i < plan.target_columns.size(); ++i) {
        for (const auto& col_def : schema) {
            if (col_def.name == plan.target_columns[i]) {
                if (values_with_ids[i].is_null && (col_def.is_not_null || col_def.is_indexed)) throw std::runtime_error("NOT NULL violation");
                if (col_def.type == DataType::STRING && !values_with_ids[i].is_null) {
                    values_with_ids[i].data = static_cast<int>(string_pool->intern(std::get<std::string>(values_with_ids[i].data)));
                } break;
            }
        }
    }
    ...
    uint32_t target_p = header->last_data_page; char page_buffer[PAGE_SIZE]; pager.read_page(target_p, page_buffer);
    uint32_t offset = *reinterpret_cast<uint32_t*>(page_buffer); if (offset == 0) offset = 4;
    if (offset + sizeof(RecordHeader) + row_bin.size() > PAGE_SIZE) {
        target_p = pager.allocate_page(); header->last_data_page = target_p; pager.read_page(target_p, page_buffer); offset = 4;
    }
    RecordHeader rh = {false, static_cast<uint32_t>(row_bin.size())};
    std::memcpy(page_buffer + offset, &rh, sizeof(rh)); std::memcpy(page_buffer + offset + sizeof(rh), row_bin.data(), row_bin.size());
    pos_t record_pos = {target_p, offset}; offset += sizeof(rh) + row_bin.size(); *reinterpret_cast<uint32_t*>(page_buffer) = offset;
    header->row_count++;
    for (size_t i = 0; i < schema.size(); ++i) {
        if (schema[i].is_indexed) {
            for (size_t j = 0; j < plan.target_columns.size(); ++j) {
    ...
    pager.write_page(0, header_buffer); pager.write_page(target_p, page_buffer);
    JournalEntry je; je.op = JournalOp::INSERT; je.table_name = plan.table_name; je.new_data = row_bin;
    auto now = std::chrono::system_clock::now(); auto timer = std::chrono::system_clock::to_time_t(now);
    std::tm bt; localtime_r(&timer, &bt); std::ostringstream oss; oss << std::put_time(&bt, "%Y.%m.%d-%H:%M:%S"); je.timestamp = oss.str();
    journal->log(je);
}
...
std::string Engine::select_records(const QueryPlan& plan) {
    ...
    Pager pager(table_path.string()); Pager idx_pager(idx_path.string());
    char hb[PAGE_SIZE]; pager.read_page(0, hb); TableHeader* h = reinterpret_cast<TableHeader*>(hb);
    auto schema = get_table_schema(plan.table_name); json res = json::array();
    if (plan.where_clause && plan.where_clause->is_leaf && plan.where_clause->op == OpType::EQ) {
        for (size_t i = 0; i < schema.size(); ++i) {
            if (schema[i].is_indexed && schema[i].name == plan.where_clause->left_column && schema[i].type == DataType::INT) {
                ...
                if (pos.is_valid()) {
                    char pb[PAGE_SIZE]; pager.read_page(pos.page_id, pb);
    ...
    for (uint32_t p = 1; p <= h->last_data_page; ++p) {
        char pb[PAGE_SIZE]; pager.read_page(p, pb); uint32_t coff = *reinterpret_cast<uint32_t*>(pb); if (coff <= 4) continue;
        size_t off = 4; while (off < coff) {
            RecordHeader* rh = reinterpret_cast<RecordHeader*>(pb + off); size_t doff = off + sizeof(RecordHeader);
            if (!rh->is_deleted) { size_t t_off = doff; auto row = RowDeserializer::deserialize(schema, pb, t_off, string_pool.get()); if (ConditionEvaluator::evaluate(row, plan.where_clause.get())) res.push_back(row); }
...
void Engine::delete_records(const QueryPlan& plan) {
    if (current_db.empty()) throw std::runtime_error("No DB");
    auto table_path = fs::path(root_path) / current_db / (plan.table_name + ".tbl");
    Pager pager(table_path.string()); char hb[PAGE_SIZE]; pager.read_page(0, hb); TableHeader* h = reinterpret_cast<TableHeader*>(hb);
    auto schema = get_table_schema(plan.table_name);
    for (uint32_t p = 1; p <= h->last_data_page; ++p) {
        char pb[PAGE_SIZE]; pager.read_page(p, pb); uint32_t coff = *reinterpret_cast<uint32_t*>(pb); if (coff <= 4) continue;
        bool changed = false; size_t off = 4; while (off < coff) {
            RecordHeader* rh = reinterpret_cast<RecordHeader*>(pb + off); size_t doff = off + sizeof(RecordHeader);
            if (!rh->is_deleted) {
                size_t t_off = doff; auto row = RowDeserializer::deserialize(schema, pb, t_off, string_pool.get());
...
void Engine::revert(const std::string& table_name, const std::string& timestamp) {
    if (current_db.empty()) throw std::runtime_error("No DB"); auto schema = get_table_schema(table_name); drop_table(table_name);
    ...
    for (const auto& entry : entries) {
        ...
        if (entry.op == JournalOp::INSERT) {
            auto table_path = fs::path(root_path) / current_db / (table_name + ".tbl"); Pager pager(table_path.string());
            char hb[PAGE_SIZE]; pager.read_page(0, hb); TableHeader* h = reinterpret_cast<TableHeader*>(hb);
            char pb[PAGE_SIZE]; pager.read_page(h->last_data_page, pb); uint32_t off = *reinterpret_cast<uint32_t*>(pb); if (off == 0) off = 4;
            if (off + sizeof(RecordHeader) + entry.new_data.size() > PAGE_SIZE) { uint32_t new_p = pager.allocate_page(); h->last_data_page = new_p; pager.read_page(new_p, pb); off = 4; }
            RecordHeader rh = {false, static_cast<uint32_t>(entry.new_data.size())};
            std::memcpy(pb + off, &rh, sizeof(rh)); std::memcpy(pb + off + sizeof(rh), entry.new_data.data(), entry.new_data.size());
            off += sizeof(rh) + entry.new_data.size(); *reinterpret_cast<uint32_t*>(pb) = off; h->row_count++;
            pager.write_page(0, hb); pager.write_page(h->last_data_page, pb);
        }
    }
}
...
std::vector<ColumnDef> Engine::get_table_schema(const std::string& table_name) {
    fs::path table_path = fs::path(root_path) / current_db / (table_name + ".tbl");
    Pager pager(table_path.string()); char buffer[PAGE_SIZE]; pager.read_page(0, buffer); TableHeader* header = reinterpret_cast<TableHeader*>(buffer);
    std::vector<ColumnDef> schema;
    for (uint32_t i = 0; i < header->column_count; ++i) {
        ColumnDef col; col.name = header->columns[i].name; col.type = (header->columns[i].type == 0) ? DataType::INT : DataType::STRING;
        col.is_not_null = header->columns[i].is_not_null; col.is_indexed = header->columns[i].is_indexed; schema.push_back(col);
    } return schema;
}
