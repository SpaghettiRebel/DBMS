...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::less<tkey>, t>::ensure_page_exists(uint32_t page_id) {
    if (!_pager) {
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::less<tkey>, t>::serialize_node(
...
template <typename tkey, typename tvalue, std::size_t t>
typename BP_tree<tkey, tvalue, std::less<tkey>, t>::bptree_node_base*
BP_tree<tkey, tvalue, std::less<tkey>, t>::deserialize_node(const char*& cur, const char* end) {
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::less<tkey>, t>::collect_leaves(
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::less<tkey>, t>::persist_to_disk() {
    if (!_pager || !_dirty) {
        return;
    }
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::less<tkey>, t>::load_from_disk() {
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::less<tkey>, t>::flush() {
    persist_to_disk();
    if (_pager) {
        _pager->file.flush();
    }
...
template <typename tkey, typename tvalue, std::size_t t>
BP_tree<tkey, tvalue, std::less<tkey>, t>::BP_tree(
    Pager* pager, uint32_t root_page_id, const std::less<tkey>& cmp, pp_allocator<value_type> alloc)
...
// Аналогично для std::greater
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::greater<tkey>, t>::ensure_page_exists(uint32_t page_id) {
    if (!_pager) {
        throw std::runtime_error("Pager not initialized for persistent B+ tree");
    }
}
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::greater<tkey>, t>::serialize_node(
    std::vector<char>& out, const bptree_node_base* node) const {
...
template <typename tkey, typename tvalue, std::size_t t>
typename BP_tree<tkey, tvalue, std::greater<tkey>, t>::bptree_node_base*
BP_tree<tkey, tvalue, std::greater<tkey>, t>::deserialize_node(const char*& cur, const char* end) {
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::greater<tkey>, t>::collect_leaves(
    bptree_node_base* node, std::vector<bptree_node_term*>& leaves) const noexcept {
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::greater<tkey>, t>::persist_to_disk() {
    if (!_pager || !_dirty) {
        return;
    }
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::greater<tkey>, t>::load_from_disk() {
    if (!_pager || _root_page_id == 0) {
        return;
    }
...
template <typename tkey, typename tvalue, std::size_t t>
void BP_tree<tkey, tvalue, std::greater<tkey>, t>::flush() {
    persist_to_disk();
    if (_pager) {
        _pager->file.flush();
    }
...
template <typename tkey, typename tvalue, std::size_t t>
BP_tree<tkey, tvalue, std::greater<tkey>, t>::BP_tree(
    Pager* pager, uint32_t root_page_id, const std::greater<tkey>& cmp, pp_allocator<value_type> alloc)
    : compare(cmp), _allocator(alloc), _root(nullptr), _size(0),
      _pager(pager), _root_page_id(root_page_id), _persistent(true), _dirty(false) {
