...
void Journal::log(const JournalEntry& entry) {
    ...
    file.write(reinterpret_cast<const char*>(&ts_len), sizeof(ts_len));
    ...
    file.write(reinterpret_cast<const char*>(&old_len), sizeof(old_len));
    file.write(entry.old_data.data(), old_len);
    file.write(reinterpret_cast<const char*>(&new_len), sizeof(new_len));
...
std::vector<JournalEntry> Journal::get_all_entries() {
    ...
    while (file.tellg() < size) {
        ...
        file.read(reinterpret_cast<char*>(&entry.record_pos), sizeof(pos_t));
