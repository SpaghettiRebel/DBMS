//организует пул из 4 фоновых потоков. Если клиент просит выполнить тяжелый запрос асинхронно, этот модуль забирает его в очередь,
//выполняет на бэкграунде и сохраняет результат, чтобы клиент не висел в ожидании
